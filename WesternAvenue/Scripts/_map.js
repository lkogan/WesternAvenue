    ...
    { 
        ...
        $scope.getData = function ()
        {
            $http.get('/home/GetAllLocation').then(function (data) {
                ...
                data.data.sortBy('ArrivalTime');
                
                if (data.data.length >= 1)
                {
                    var firstRecord = data.data[0];

                    $scope.markers.push
                        ({
                            id: firstRecord.LocationID,
                            coords:
                            {
                                latitude: firstRecord.Lat,
                                longitude: firstRecord.Long
                            },
                            title: firstRecord.title, //title of the makers  
                            address: firstRecord.Address, //Address of the makers  
                            image: firstRecord.ImagePath, //image --optional  
                ...
                for (var i = 0; i < data.data.length; i++) {
                    ...
                    if (i < 5)
                    {
                        console.log(data.data[i].ArrivalTime);
                    }
